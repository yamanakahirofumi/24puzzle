...
- **キーボード操作**：
    ...
    - **マッピング**：
        - `↑` (Up)：空きマスを上に移動させる（空きマスの**上**にあるパネルが下に移動する）。
        - `↓` (Down)：空きマスを下に移動させる（空きマスの**下**にあるパネルが上に移動する）。
        - `←` (Left)：空きマスを左に移動させる（空きマスの**左**にあるパネルが右に移動する）。
        - `→` (Right)：空きマスを右に移動させる（空きマスの**右**にあるパネルが左に移動する）。
