...
### 1.3 名前付け規則
- クラス名は `UpperCamelCase`（例：`PuzzleBoard`）を使用します。
- メソッド名、変数名は `lowerCamelCase`（例：`shuffleTiles`）を使用します。
...
- **Import 文のソートとグルーピング**：
  - 以下の順序で記述し、各グループ（1〜5）の間には空行を挿入してください。
    1. プロジェクト固有のクラス
    2. 3rdライブラリ等の他クラス
    3. `javax.*`
    4. `java.*`
    5. `import static` (すべての static import)
